...
class TextAnalysisPipeline:
    def __init__(self):
        try:
            self.cohere_api_key = "9YTlH1nt6mTdpmtHkNTXR9zrcnwCE5JqZqhekXdk"
            self.pinecone_api_key = "a90dc415-ec36-40f6-bbed-d0066ea96383"
            ...
            if self.index_name not in pinecone.list_indexes():
                pinecone.create_index(name=self.index_name, metric="cosine", dimension=1024)
...
Remember, your goal is to assist users in understanding the content of the file they've uploaded."""
    ...
    def reset_pipeline(self):
        try:
            ...
            self.vectorstore = None
            self.current_file = None
            logger.info("Pipeline reset: Deleted all vectors from Pinecone and reset current file")
        except Exception as e:
        ...
        try:
            ...
            for i in range(0, len(docs), batch_size):
                ...
                if self.vectorstore is None:
                    self.vectorstore = Pinecone.from_documents(batch, self.embeddings, index_name=self.index_name)
                else:
                    time.sleep(60)
                    self.vectorstore.add_documents(batch)
                logger.info(f"Processed batch {i//batch_size + 1} of {len(docs)//batch_size + 1}")
    ...
    def query(self, query):
        try:
            ...
            retrieved_text = retrieved_documents[0].page_content if retrieved_documents else "No relevant documents found."
